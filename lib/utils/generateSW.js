...
const { getBuildServiceWorkerFilePath } = require("../configs/paths");
const userConfig = require("../configs/userConfig");

exports.generateServiceWorkerFile = async function (config) {
  const filePath = getBuildServiceWorkerFilePath();

  try {
    // Extract assets from HTML files using AssetExtractor
    let filesToCache = await new AssetExtractor(userConfig.outDir).runExtractor(
      config.htmlFiles
    );
    ...
    const message = "Service Worker file successfully generated at:" + filePath;
    return message;
  } catch (error) {
